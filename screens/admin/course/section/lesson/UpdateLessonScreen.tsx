...
const UpdateLessonScreen: React.FC<MyScreenProps['UpdateLessonScreenProps']> = ({
  navigation,
  route,
}) => {
  const { sectionData, lessonData, onLessonUpdated } = route.params;

  const [formData, setFormData] = useState({
    title: lessonData.title,
    content: lessonData.content,
    is_quizz: lessonData.is_quizz,
    video_url: lessonData.video_url,
  });

  const [questions, setQuestions] = useState<Question[]>(lessonData.questions);
  const [newIdQuestion, setNewIdQuestion] = useState(lessonData.newIdQuestion);
  ...
  const handleSubmit = () => {
    ...
    if (formData.is_quizz && questions.length === 0) {
    ...
    return (
      <View style={styles.questionsContainer}>
        <View style={styles.questionHeader}>
          ...
          <TouchableOpacity style={styles.addButton} disabled={loading} onPress={addNewQuestion}>
